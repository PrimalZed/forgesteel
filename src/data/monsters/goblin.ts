...
Their long arms and prehensile toes make goblins particularly well adapted to environments requiring climbing, and they live equally comfortably in treetop cities as in subterranean realms filled with stalactites, ledges, and chasms. However, just as many prefer to dwell in tight-knit neighborhoods within diverse cities. Many of those goblins who cross swords with adventuring heroes are the worst of their society—thieves and murderers shunned by their own people.`,
	...
	monsters: [
		...
		},
		{
			...
			description: '',
			level: 1,
			role: MonsterLogic.createRole(MonsterRoleType.Brute, 'minion'),
			keywords: [ 'Goblin', 'Humanoid' ],
			encounterValue: 6,
			size: MonsterLogic.createSize(1, 'S'),
			speed: MonsterLogic.createSpeed(5, 'climb'),
		...
		},
		{
			...
			description: '',
			level: 1,
			role: MonsterLogic.createRole(MonsterRoleType.Hexer, 'minion'),
			keywords: [ 'Animal', 'Goblin' ],
			encounterValue: 6,
			size: MonsterLogic.createSize(1, 'T'),
			speed: MonsterLogic.createSpeed(5, 'fly'),
			stamina: 3,
			stability: 0,
			freeStrikeDamage: 1,
			withCaptain: 'Speed +3',
		...
		},
		{
			...
			description: '',
			level: 1,
			role: MonsterLogic.createRole(MonsterRoleType.Brute, 'band'),
			keywords: [ 'Goblin', 'Humanoid' ],
			encounterValue: 3,
			size: MonsterLogic.createSize(1, 'S'),
			speed: MonsterLogic.createSpeed(6, 'climb'),
		...
		},
		{
			...
			description: '',
			level: 1,
			role: MonsterLogic.createRole(MonsterRoleType.Hexer, 'band'),
			keywords: [ 'Goblin', 'Humanoid' ],
			encounterValue: 3,
			size: MonsterLogic.createSize(1, 'S'),
			speed: MonsterLogic.createSpeed(5, 'climb'),
			...
			features: [
				FeatureLogic.feature.createAbilityFeature({
					ability: AbilityLogic.createAbility({
						...
						distance: [ AbilityLogic.distance.create({ type: AbilityDistanceType.Cube, value: 3, within: 15 }) ],
						...
						spend: [
							// TODO: "1+" to spend 1 or more malice, as described
							{ value: 1, effect: 'Increase the slide for one target by 2 squares for each malice spent.' }
						]
					})
				}),
				FeatureLogic.feature.createAbilityFeature({
		...
		},
		{
			...
			description: '',
			level: 1,
			role: MonsterLogic.createRole(MonsterRoleType.Harrier, 'band'),
			keywords: [ 'Goblin', 'Humanoid' ],
			encounterValue: 3,
			size: MonsterLogic.createSize(1, 'S'),
			speed: MonsterLogic.createSpeed(6, 'climb'),
			...
			features: [
				...
				FeatureLogic.feature.createAbilityFeature({
					ability: AbilityLogic.createAbility({
						...
						type: AbilityLogic.type.createManeuver(),
						cost: 1,
						distance: [ AbilityLogic.distance.createRanged(20) ],
						target: 'Special',
				...
				FeatureLogic.feature.createAbilityFeature({
					ability: AbilityLogic.createAbility({
						...
						name: 'Meat Shield!',
						type: AbilityLogic.type.createTrigger('A creature targets the monarch with a strike.'),
						distance: [ AbilityLogic.distance.createMelee(1) ],
						target: 'One ally',
		...
		},
		{
			...
			description: '',
			level: 1,
			role: MonsterLogic.createRole(MonsterRoleType.Mount, 'troop'),
			keywords: [ 'Animal', 'Goblin' ],
			encounterValue: 12,
			...
			features: [
				...
				FeatureLogic.feature.createAbilityFeature({
					ability: AbilityLogic.createAbility({
						...
						name: 'Skitter',
						type: AbilityLogic.type.createTrigger('The spider or an ally riding the spider is targeted by an ability.'),
						distance: [ AbilityLogic.distance.createSelf() ],
						target: 'Self',
		...
		},
		{
			...
			description: '',
			level: 1,
			role: MonsterLogic.createRole(MonsterRoleType.Mount, 'band'),
			keywords: [ 'Animal', 'Goblin' ],
			encounterValue: 3,
			size: MonsterLogic.createSize(1, 'L'),
